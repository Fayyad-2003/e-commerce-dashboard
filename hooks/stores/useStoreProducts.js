...
export default function useStoreProducts(subCategoryId) {
    ...
    const apiUrl = useMemo(() => {
        ...
        return `/api/stores/sub-categories/${subCategoryId}/products?${qs.toString()}`;
    }, [subCategoryId, page, perPage]);
    ...
    const reload = useCallback(async () => {
        ...
        try {
            ...
            setErr(e?.message || "تعذّر جلب المنتجات");
        } finally {
            setLoading(false);
        }
    }, [apiUrl, page, perPage]);
    ...
    const goToPage = (n) => {
        ...
        router.push(`/admin/stores/products/${subCategoryId}?${sp.toString()}`);
    };
